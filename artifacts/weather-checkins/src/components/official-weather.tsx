...
import { Loader2, Wind, Thermometer } from "lucide-react";

export function OfficialWeather() {
  const { temperature, windSpeed, type, loading, error } = useWeather();
  ...
  if (error) {
    return (
      <div className="bg-card rounded-3xl p-8 border border-border shadow-sm flex flex-col items-center justify-center min-h-[200px] text-center">
        ...
        <p className="text-muted-foreground text-sm">We are using fallback data or there is a connection issue.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card rounded-3xl p-8 border border-border shadow-sm relative overflow-hidden">
      ...
      <div className="flex flex-col items-center text-center relative z-10">
        ...
        <div className="flex items-center gap-6 mt-6 mb-8 text-lg font-medium text-foreground">
